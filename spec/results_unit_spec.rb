  ...
  describe Results::Bad do
    ...
    describe '#zip' do
      ...
      context 'when other is a single bad' do
        subject { bad.zip(bad) }
        it { is_expected.to eq Results::Bad.new([bad.why.because, bad.why.because]) }
      end
      ...
      context 'when self and/or other is a multiple bad' do
        ...
        it { expect(bad.zip(multi_bad)).to       eq Results::Bad.new([bad.why.because] + multi_why) }
        it { expect(multi_bad.zip(bad)).to       eq Results::Bad.new(multi_why + [bad.why.because]) }
